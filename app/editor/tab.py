...
# ── high-fidelity text-edit reinsertion ─────────────────────────────────────
# When the user edits an existing text span the old code redacted the span with
# an opaque WHITE rectangle and re-typed the text with a base-14 font at an
# inflated size (``max(size, bbox_height)``), collapsing the family and dropping
# bold/italic. The helpers below reproduce the original size, weight, colour and
# — when the source font is fully embedded (non-subset) and covers the new
# glyphs — the exact typeface, using a transparent redaction (no white box) plus
# ``Page.insert_htmlbox`` (HarfBuzz shaping + Noto glyph fallback). Everything is
# wrapped so a failure degrades gracefully to a base-14 ``insert_text`` instead
